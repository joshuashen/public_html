  ...
  def readGT(cfile)
    ...
    fh.each do |line|
      ...
      if cols[0] == 'rs#' # first line
        ...
        @snps << rs.strip
        @firstGTCol.upto(cols.size - 1) do |i| 
##           puts "#{nameHash[i]}\t#{i}\t#{cols[0..10].join(' ')}"
          @genotype[nameHash[i]][rs] = cols[i].tr('N', '0').tr('-', '0').tr('_', '0')
        end
      end
    end
    fh.close
...
if __FILE__ == $0 
  ...
  gt = Genotype.new(gtFile, snpMap, 11)
  gt.output(prefix)
end
